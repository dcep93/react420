...
export default function Employees() {
  ...
  const getActive = (t: number) =>
    data.filter((d) => d.start <= t && d.end >= t);
  ...
  const metrics: {
    ...
    },
    {
      ...
      f: (t) =>
        ((here_now) =>
          here_now.filter(
            ({ end }) => end >= t + (months * 60 * 60 * 24 * 365) / 12
          ).length / here_now.length)(getActive(t)),
      g: () => (
        <span>
          <input
            type={"range"}
    ...
    },
    {
      ...
      f: (t) =>
        ((here_now) =>
          (t -
            here_now[Math.floor((here_now.length - 1) * (1 - percentile))]
              ?.start) /
          (60 * 60 * 24 * 365))(getActive(t).sort((a, b) => a.start - b.start)),
      g: () => (
        <span>
          <input
            type={"range"}
